...
if args.command == 'Dir':
	...
	def threadFunction(url, str_list, orianted_list):
		...
		if(extentions is not None):
			...
			orianted_list_len = len(orianted_list)
			if(count > 2):
				num = orianted_list_len / 2
				orianted_list = orianted_list[int(num):]
			...
			#print(new_result_folder)
			#print(recyle_folder_list)
			if(len(new_result_folder) > 0):
				...
				for folderUrl in recyle_folder_list:
					threadFunction(folderUrl, [], orianted_folder_list)
